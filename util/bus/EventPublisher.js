...
EventPublisher.prototype.publishNewPosition = function(pet,currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.NEW_POSITION,pet,currentLocationReport);
};

EventPublisher.prototype.publishVFBreachEvent = function(pet,currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.VIRTUAL_FENCE_BREACH,pet,currentLocationReport);
};

EventPublisher.prototype.publishVFEnterEvent = function(pet,currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.VIRTUAL_FENCE_ENTER,pet,currentLocationReport);
};

EventPublisher.prototype.publishLowBatteryEvent = function(pet,currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.LOW_BATTERY,pet,currentLocationReport);
};

EventPublisher.prototype.publishVFBreachCancelEvent = function(pet,currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.VF_BREACH_CANCEL,pet,currentLocationReport);
};

EventPublisher.prototype.publishTrackingSessionStartEvent = function(pet,currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.TRACKING_SESSION_START,pet,currentLocationReport);
};

EventPublisher.prototype.publishFoundPetEvent = function(pet,currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.PET_FOUND,pet,currentLocationReport);
};

EventPublisher.prototype.publishLookingForPetEvent = function(pet,currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.LOOKING_FOR_PET,pet,currentLocationReport);
};

EventPublisher.prototype.publishLocatePetEvent = function(pet) {
    this.publishEvent(gibiconstants.EVENT_TYPES.LOCATE_PET,pet,null);
};

EventPublisher.prototype.publishSleepModeRequestEvent = function(pet,sleepModeArgs,currentLocationReport) {
    var event            = this.buildBaseEvent(gibiconstants.EVENT_TYPES.SLEEP_MODE_REQUEST,pet,currentLocationReport);
...
EventPublisher.prototype.publishChangeReportingFrequencyEvent = function(pet) {
    this.publishEvent(gibiconstants.EVENT_TYPES.CHANGE_REPORTING_FREQUENCY,pet,null);
};
...
EventPublisher.prototype.publishChangeReportingFrequencyReplyEvent = function(bgi,publisher) {
    ...
    if ( bgi.status )
        evt = gibiconstants.EVENT_TYPES.CHANGE_REPORTING_FREQUENCY_SUCCESS;
    else
        evt = gibiconstants.EVENT_TYPES.CHANGE_REPORTING_FREQUENCY_FAIL;
...
EventPublisher.prototype.publishLocatePetSuccessEvent = function(pet, currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.LOCATE_PET_RESULT_SUCCESS,pet,currentLocationReport);
};

EventPublisher.prototype.publishLocatePetFailureEvent = function(pet, currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.LOCATE_PET_RESULT_FAIL,pet,currentLocationReport);
};

EventPublisher.prototype.publishSharePetLocationSuccessEvent = function(pet, currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.SHARE_PET_LOCATION_RESULT_SUCCESS,pet,currentLocationReport);
};

EventPublisher.prototype.publishSharePetLocation = function(pet, currentLocationReport, contacts) {
    var event       = this.buildBaseEvent(gibiconstants.EVENT_TYPES.SHARE_PET_LOCATION,pet,currentLocationReport);
    event.contacts  = contacts;
    this.reallyPublishEvent(event);
};

EventPublisher.prototype.publishSharePetLocationFailureEvent = function(pet, currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.SHARE_PET_LOCATION_RESULT_FAIL,pet,currentLocationReport);
};

EventPublisher.prototype.publishPowerUpEvent = function(pet, currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.POWER_UP,pet,currentLocationReport);
};

EventPublisher.prototype.publishPowerDownEvent = function(pet, currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.POWER_DOWN,pet,currentLocationReport);
};

EventPublisher.prototype.publishFullyChargedEvent = function(pet,currentLocationReport) {
    this.publishEvent(gibiconstants.EVENT_TYPES.FULLY_CHARGED,pet,currentLocationReport);
}
...
EventPublisher.prototype.publishResetPassword = function(user, password) {
    var event        = {};
    event.eventType       = gibiconstants.EVENT_TYPES.RESET_PASSWORD;
    event.user       = user;
